...
- 说明所选结构的理论 / 方法来源及本 Skill 的改编；只写作者或方法名不算可追溯，至少写出一个完整的 `https://` 链接；
...
默认回复不得原样显示 `DESIGN`、`AUDIT`、`REBUILD`、`payoff`、`main`，也不得叙述仓库、分支、文件路径、模式名、状态码或读取过程。确需表达时分别写成“结构设计”“独立审查”“结构重建”“观看回报”“当前真源”，并只说明它们对创作下一步的影响。
